    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          { label: t('denials.pending'), value: (data?.items ?? []).filter(d => !d.is_resolved).length, icon: AlertTriangle, color: 'text-amber-500 bg-amber-50' },
          { label: t('denials.resolved'), value: (data?.items ?? []).filter(d => d.is_resolved).length, icon: CheckCircle, color: 'text-emerald-500 bg-emerald-50' },
